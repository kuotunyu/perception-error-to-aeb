...
THE ANIMATION IS GENERATED BY PYTHON. The read-only JavaScript status adapter
is checked separately by tests/js/check-replay-status.cjs; it is not a player.
"""
...
def test_replay_workspace_separates_context_and_preserves_track_access(tmp_path: Any) -> None:
    ...
    assert "<h1>情境重播</h1>" in html
    assert 'class="replay-status"' in html
    assert 'id="replay-time"' in html
    assert 'id="replay-speed"' in html
    assert 'id="replay-ttc"' in html
    assert "plotly_animatingframe" in html
    assert "回放" not in html.split("<style>")[0]
...
def test_replay_status_contract_fixture(tmp_path: Any) -> None:
    """Export real generated JS for the required independent Node contract check.

    Missing serialization, wrong formatting, and lost slider targets are bugs.
    Node checks missing event updates and stale display; this is not browser QA.
    """
    import dataclasses
    import json
    import re
    ...
    frames = tuple(
        dataclasses.replace(
            frame(index, state="monitor" if index == 0 else "full_brake"),
            ego_speed_mps=10.0 if index == 0 else 3.5,
            ttc_s=None if index == 0 else 1.25,
        )
        for index in range(151)
    )
    expected = {
        f"{index / 10:.1f}": (
            ["0.0 s · monitor", "10.0 m/s", "無可用數值"]
            if index == 0
            else [f"{index / 10:.1f} s · full_brake", "3.5 m/s", "1.25 s"]
        )
        for index in range(151)
    }
    path = tmp_path / "replay.html"
    replay.write_replay_html(frames, "synthetic / oracle_aeb", path)
    html = path.read_text(encoding="utf-8")
    script = re.search(r"const replayStates = .*?\n\s*\}\);", html, re.DOTALL)
    assert script is not None
    serialized = re.search(r"const replayStates = (.*);", script[0])
    assert serialized is not None
    assert json.loads(serialized[1]) == expected
    figure = replay.build_replay_figure(frames, "synthetic / oracle_aeb")
    steps = figure.layout.sliders[0].steps
    assert len(figure.frames) == len(steps) == 151
    assert all(step.method == "animate" for step in steps)
    targets = [step.args[0][0] for step in steps]
    assert targets == list(expected)
    (tmp_path / "contract.json").write_text(
        json.dumps({"script": script[0], "expected": expected, "slider_targets": targets}),
        encoding="utf-8",
    )
